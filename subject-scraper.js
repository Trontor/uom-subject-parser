...
const getHTML = (URL, callback) => {
  ...
  customHeaderRequest.get(URL, function(error, response, body) {
    callback(body);
  });
};
...
const scrapePage = (year, studyPeriod, baseURL, number, maxPages, callback, finishedCallBack) => {
  ...
  getHTML(pageURL, html => {
    ...
    list.each(function() {
      const subjectCode = $(this).find(".search-results__accordion-code").text().trim();   
      if (allSubjects.filter(val => val.code === subjectCode).length != 0) {
        //console.log(`\t|-> ${subjectCode} already exists, skipping!`);
        return;
      }
      const title = $(this).find(".search-results__accordion-title").text().replace(subjectCode, "").trim();
      const details = $(this).find(".search-results__accordion-detail").text().trim();
...
const scrapeSubjects = (year, studyPeriod, finishedCallBack) => {
  ...
  try {
    ...
    console.log(`Read existing subject file ${allSubjects.length} entires existing.`); 
  } catch (error){
...
const invokePageScraper = (year, studyPeriod, baseURL, finishedCallBack, maxCount, pageNum = 1)  => {
  ...
  scrapePage(
    ...
    pageSubjects => {
      ...
      if (pageNum != maxCount){
        invokePageScraper(year, studyPeriod, baseURL, finishedCallBack, maxCount, ++pageNum);
      }
